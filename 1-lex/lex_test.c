...
void print_char(char c){
    switch(c){
        case '\0':  fprintf(stdout, "%c", "\\0");  break; 
        case '\'':  fprintf(stdout, "%c", "\\'");  break;  
        case '\"':  fprintf(stdout, "%c", "\\\""); break;
        case '\?':  fprintf(stdout, "%c", "\\?");  break;
        case '\\':  fprintf(stdout, "%c", "\\\\"); break;
        case '\a':  fprintf(stdout, "%c", "\\a");  break;
        case '\b':  fprintf(stdout, "%c", "\\b");  break; 
        case '\f':  fprintf(stdout, "%c", "\\f");  break;
        case '\n':  fprintf(stdout, "%c", "\\n");  break; 
        case '\r':  fprintf(stdout, "%c", "\\r");  break;
        case '\t':  fprintf(stdout, "%c", "\\t");  break;
        case '\v':  fprintf(stdout, "%c", "\\v");  break; 
        default:
            if(c >127 || c < 32){
...
//                if(yylval.str.len == 3 && yylval.str.string_literal[1] < 127){
//                    fprintf(stdout, "%s\t %d\t CHARLIT\t %c\n",filename, lineno,  yylval.str.string_literal[1]);
//                }else if(yylval.str.len > 3 && yylval.str.string_literal[1] != '\\'){
//                    fprintf(stderr, "%s:%d\t Error: char constant too long\n", filename, lineno);
//                    fprintf(stdout, "%s\t %d\t CHARLIT\t %c\n",filename, lineno,  yylval.str.string_literal[1]);
//                }else{
