...
%%
finshed_unit:         
    translation_unit                                  {/* migrate asmgen here */}
    ;

/* (6.9) */
translation_unit:     
      extern_declaration                                //{print_sym(curr_scope); /* Nothing to do here ¯\_(ツ)_/¯ */ } 
    | translation_unit extern_declaration               //{print_sym(curr_scope); /* Nothing to do here ¯\_(ツ)_/ */ }
    ;

/* (6.9) */
extern_declaration:   
      declaration                                       {sym_declaration($1, curr_scope);}
    | func_def                                          {sym_func_def($1, curr_scope); 
                                                            /* generate quads for function definition */
                                                        //  print_sym(curr_scope);
                                                            struct bblock_list * quads = gen_quads($1);
                                                            gen_asm(quads, $1);
                                                        }
...
prim_expr:            
      IDENT                         {
                                        $$=alloc_ident($1);
                                        struct sym_entry *  temp = alloc_sym_ent($1, ENT_VAR, NS_MISC);
...
/* (6.5.16) */
assign_expr:        
      unary_expr '=' assign_expr                {$$=alloc_binary(ASSIGN, $1, '=', $3);} 
...
unary_expr:         
      postfix_expr                              {$$=$1;}
    | PLUSPLUS unary_expr                       {struct astnode *temp = alloc_num(1, 0.0, N_INT, N_SIGNED);
                                                    $$=alloc_and_expand_assignment($2, '+', temp);}
    | MINUSMINUS unary_expr                     {struct astnode *temp = alloc_num(1, 0.0, N_INT, N_SIGNED);
                                                    $$=alloc_and_expand_assignment($2, '-', temp);}
...
/* (6.7) */
declaration:        
      declaration_specs ';'                         {$$=alloc_declaration($1, NULL);}
...
/* (6.7.1) */
stg_class_spec:     
      AUTO                                      {$$=alloc_decl_spec(STG_AUTO, AST_DECL_STG);    }
...
/* (6.7.2) */
type_spec:          
      VOID                                          {$$=alloc_decl_spec(TYPE_VOID, AST_DECL_TYPE_SPEC);    } 
...
/* (6.7.2.1) */
struct_union_spec:  
      struct_union '{' struct_declaration_list '}'              {$$=$1; sym_struct_define($1, $3);}
    ...
    | struct_union IDENT                                        { /* either a forward declartion or getting the type (I will be ignoring/not handling forward declarations) */
                                                                    struct sym_entry *  temp = alloc_sym_ent($2, ENT_SU_TAG, NS_SU);
...
/* (6.7.2.1) */
struct_union:       
      STRUCT    {$$=alloc_st_un(AST_STRUCT, curr_scope->scope_type);}
    | UNION     {$$=alloc_st_un(AST_UNION, curr_scope->scope_type);}
    ;

/* (6.7.2.1) */
struct_declaration_list:    
      struct_declaration                            {$$=alloc_list($1);}
    | struct_declaration_list struct_declaration    {$$=list_append_elem($2, $1);}
    ;

/* (6.7.2.1) */
struct_declaration: 
      specific_qualif_list struct_decl_list ';'         {$$=alloc_declaration($1, $2);}
    ;

/* (6.7.2.1) */
specific_qualif_list:   
      type_spec                         {$$=alloc_list($1);}
...
type_qualif:        
      CONST                                     {$$=alloc_decl_spec(QUALIF_CONST, AST_DECL_TYPE_QUALIF);}
...
/* (6.7.4) */
func_spec:          
      INLINE                                    {$$=alloc_decl_spec(FUNC_INLINE, AST_DECL_FUNC_SPEC);}       
    ;
...
*/
direct_decl:        
      IDENT                                         {$$=alloc_list(alloc_ident($1));} 
    | '(' decl ')'                                  {$$=$2;}
...
/* (6.7.5): ignoring type qualifieres, but support in grammar */
pointer:            
      '*'                                           {$$=alloc_list(alloc_ptr(NULL));} 
...
/* (6.7.5): not REALLLY handling this */
type_qualif_list:   
      type_qualif                               
...
/* (6.7.5) */
param_list:         
      param_declaration                         {$$=alloc_list($1);}
    | param_list ',' param_declaration          {$$=list_append_elem($3, $1);}
    ;

/* (6.7.5) */
param_declaration:  
      declaration_specs decl                    {$$=alloc_declaration($1, $2);}
...
label_stmnt:        
      IDENT ':' statement                       { 
                                                    $$=alloc_label_stmnt(AST_LABEL, $3, $1, NULL);
...
/* (6.8.2) */
block_item_list:    
      block_item                    {$$=alloc_list($1);}
    | block_item_list block_item    {$$=list_append_elem($2, $1);}
    ;

/* (6.8.2) */
block_item:         
      declaration                   {$$=$1; sym_declaration($1, curr_scope);}
    | statement                     {$$=$1;}
    ;
...
/* (6.8.4) */
select_stmnt:       
      IF '(' expr ')' statement                 {$$=alloc_select_stmnt(AST_IF_STMNT, $3, $5, NULL);} 
...
/* (6.8.5) */
jump_stmnt:         
      GOTO IDENT ';'                        {$$=alloc_jump_stmnt(AST_GOTO, $2, NULL);}         
